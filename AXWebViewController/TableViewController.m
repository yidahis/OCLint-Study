...
@interface TableViewController () <UITextFieldDelegate,AXWebViewControllerDelegate>
@property (nonatomic, assign) NSTimeInterval beginData;
@end
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    switch (indexPath.row) {
        ...
        case 4: {
            AXWebViewController *webVC = [[AXWebViewController alloc] initWithAddress:@"https://websets.yongqianbao.com/activities.html?utoken=IjE3NjAwMjIzODExIg:1hVXb4:cG3VC3ecbde_66HCblF8WirWJw4&token=IjE3NjAwMjIzODExIg:1hVXb4:cG3VC3ecbde_66HCblF8WirWJw4&ac_token=InlxYi4xNzYwMDIyMzgxMS4yNzQ0Ig:1hVXb4:iKx48cGt7hEHVJmw2w0JiLz1Nto#/201811_02"];
            webVC.showsToolBar = NO;
            webVC.delegate = self;
            if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_9_0) {
                webVC.webView.allowsLinkPreview = YES;
...
- (void)webViewControllerDidStartLoad:(AXWebViewController *)webViewController{
    self.beginData =  [[NSDate date]  timeIntervalSince1970];
}
...
- (void)webViewControllerDidFinishLoad:(AXWebViewController *)webViewController{
    ...
    NSTimeInterval time =  endDate - self.beginData;
    NSLog(@"total Time : %f",time);
}
